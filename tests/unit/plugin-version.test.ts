...
describe('plugin manifest version', () => {
    ...
    it('is synced by scripts/sync-plugin-version.mjs with repository formatting', async () => {
        const workDir = fs.mkdtempSync(path.join(os.tmpdir(), 'plugin-version-'))
        try {
            fs.copyFileSync(path.join(PROJECT_ROOT, '.prettierrc.json'), path.join(workDir, '.prettierrc.json'))
            ...
            fs.writeFileSync(
                ...
                JSON.stringify({ name: 'vue-ts-lsp', version: '0.0.1', description: 'x', keywords: ['lsp', 'vue', 'typescript', 'language-server'] }, null, 4) +
                    '\n'
            )
            ...
            const filepath = path.join(workDir, '.claude-plugin', 'plugin.json')
            expect(await check(fs.readFileSync(filepath, 'utf8'), { ...(await resolveConfig(filepath)), filepath })).toBe(true)
        } finally {
